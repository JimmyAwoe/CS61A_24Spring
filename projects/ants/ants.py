...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if self.is_container == True and self.ant_contained == None and place.ant.is_container == False: 
                self.store_ant(place.ant)
                place.ant = self
            elif place.ant.is_container == True and place.ant.ant_contained == None and self.is_container == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        count = 0
        while place_now.is_hive != True and count <= self.upper_bound:
            if count < self.lower_bound:
                count += 1
                place_now = place_now.entrance
                continue
            if len(place_now.bees) > 0:
                return random.choice(place_now.bees)
            place_now = place_now.entrance
            count += 1
...
class FireAnt(Ant):
    ...
    def damage_all_bee(self, bees_list, damage):
        bl_copy = bees_list[:]
        for bee in bl_copy:
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount > 0:
            self.damage_all_bee(self.place.bees, amount)
            Ant.reduce_health(self, amount)
        else:
            self.damage_all_bee(self.place.bees, amount + self.damage)
            Ant.reduce_health(self, amount)
        # END Problem 5
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        FireAnt.damage_all_bee(self, self.place.bees, self.damage)
    ...
    def action(self, gamestate):
        ...
        while now_place != None:
            if now_place.ant != None:
                ...
                if now_place.ant.is_container == True and now_place.ant.ant_contained != None:
