...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        lambdaproc = do_lambda_form(Pair(signature.rest, expressions.rest), env) 
        env.define(signature.first, lambdaproc)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    lambdaproc = LambdaProcedure(formals, expressions.rest, env)
    return lambdaproc
    # END PROBLEM 7
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        if validate_form(bindings.first.rest, 1, 1):
            return SchemeError
        names = Pair(bindings.first.first, names)
        eval_val = scheme_eval(bindings.first.rest.first, env)
        print('DEBUG:', eval_val)
        vals = Pair(eval_val, vals)
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    muproc = MuProcedure(expressions.first, expressions.rest)
    return muproc
    # END PROBLEM 11
