...
def make_change(amount, coins):
    ...
    if smallest == amount:
        ...
        if isinstance(make_change(amount - smallest, rest), list):
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        change_combination = make_change(coin, dict(self.coins))
        if change_combination == None:
            return [coin]
        for i in change_combination:
            self.coins[i] = self.coins[i] - 1
            if self.coins[i] == 0:
                self.coins.pop(i)
